"""
Unified IDX Stock Insight Engine
Uses shared technical analysis library
Sustainable solution for both agent and dashboard
"""
...
# Import shared technical analysis library
from idx_technical_analysis import IDXTechnicalAnalysis

# --- UI CONFIGURATION ---
st.set_page_config(page_title="🏛️ IDX Master Trading Terminal", layout="wide")
...
# Initialize shared technical analysis
ta = IDXTechnicalAnalysis()
    ...
    with st.spinner(mode_title):
        ...
        if isinstance(history, str):
            ...
            for symbol_jk in available_symbols:
                ...
                # --- TECHNICALS & RISK (USING SHARED LIBRARY) ---
                df['returns'] = df['close'].pct_change()
                volatility = ta.calculate_volatility(df['close'])
                risk_label = ta.get_risk_level(volatility)

                # Use shared technical analysis
                df['RSI_14'] = ta.calculate_rsi(df['close'], 14)
                df['SMA_50'] = ta.calculate_sma(df['close'], 50)
                
                # Secondary safeguard to ensure RSI and SMA defaults if NaNs slip through
                ...
                # Volume sentiment using shared analysis
                f_flow = ta.analyze_volume_sentiment(df)
                
                # --- STRICT RECOMMENDATION LOGIC (USING SHARED LIBRARY) ---
                rec = ta.get_recommendation(rsi_val, is_elite)
            ...
            with st.expander(L["method"], expanded=False):
                ...
                st.write("**🔧 Unified System:** Uses shared technical analysis library for consistency with autonomous agent.")
                st.write("**🚀 Sustainable:** No external dependencies, manual calculations only.")
